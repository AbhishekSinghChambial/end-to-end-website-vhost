...
for i in $(cat env-setups-details.txt); do 
    ...
    if [ -z "$setupcompleted" ]; then
        ...
        scp $i.* ubuntu@54.197.66.70:/tmp/
        ## create document root directory
        echo "create doc root dir on server"
        ssh -t ubuntu@54.197.66.70 sudo mkdir /var/www/html/$i
        ## move conf and web file to their location

        echo "move conf and html file"
        ssh -t ubuntu@54.197.66.70 sudo mv /tmp/$i.conf /etc/apache2/sites-enabled/$i.conf
        ssh -t ubuntu@54.197.66.70 sudo mv /tmp/$i.html /var/www/html/$i/index.html
        ## restart apache 
        echo "reload apache"
        ssh -t ubuntu@54.197.66.70 sudo systemctl reload apache2
